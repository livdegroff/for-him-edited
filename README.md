# for-him-edited
<!DOCTYPE html>

<html>

<head>

  <title>For Him 💖</title>

  <link rel="stylesheet" href="style.css">

</head>

<body>

<div class="grid-container">

  <!-- MESSAGE -->

  <div class="message box">

    <h2>New Message 💌</h2>

    <p>

      hey handsome 😭💖

    </p>

  </div>

  <!-- BOREDOM BUTTON -->

  <div class="boredom box">

    <h2>Boredom Button 🎲</h2>

    <button onclick="generateChallenge()">

      🎲

    </button>

    <p id="challenge"></p>

  </div>

  <!-- CHECKLIST -->

  <div class="checklist box">

    <h2>Daily Checklist ✅</h2>

    <label><input type="checkbox"> Drink Water 💧</label><br>

    <label><input type="checkbox"> Eat Food 🍔</label><br>

    <label><input type="checkbox"> Text Meeee 💌</label><br>

    <label><input type="checkbox"> Go Outside 🌱</label><br>

    <label><input type="checkbox"> Stretch 🧘</label>

  </div>

  <!-- MINI GAMES -->

  <div class="games box">

    <h2>Mini Games 🕹</h2>

    <ul>

      <li>

        <a href="https://wordleunlimited.org/" target="_blank">

          Wordle

        </a>

      </li>

      <li>

        <a href="https://skribbl.io/" target="_blank">

          Skribbl.io

        </a>

      </li>

      <li>

        <a href="https://www.chess.com/" target="_blank">

          Chess

        </a>

      </li>

      <li>

        <a href="https://www.geoguessr.com/" target="_blank">

          GeoGuessr

        </a>

      </li>

    </ul>

  </div>

  <!-- QUIZZES -->

  <div class="quizzes box">

    <h2>Quizzes 🧠</h2>

    <ul>

      <li>

        <a href="PASTE_GOOGLE_FORM_LINK">

          How Well Do You Know Me?

        </a>

      </li>

      <li>

        <a href="PASTE_GOOGLE_FORM_LINK">

          Boyfriend Test

        </a>

      </li>

      <li>

        <a href="PASTE_GOOGLE_FORM_LINK">

          Which Version Of Me Are You Dating?

        </a>

      </li>

    </ul>

  </div>

  <!-- MAIL -->

  <div class="mail box">

    <h2>You’ve Got Mail 💌</h2>

    <div class="container" onclick="openLetter()">

      <div class="envelope"></div>

      <div class="letter">

        <p>

          hi handsome 💖<br><br>

          i hope you’re having a good day.<br>

          drink water and remember i love you 😭

        </p>

      </div>

    </div>

  </div>

</div>

<script src="script.js"></script>

</body>

</html>